...
function ViewModal({ martyr, onClose }) {
  ...
  const Row = ({ label, value }) => (
    <div className="flex flex-col gap-0.5">
      <span className="text-[9px] font-black tracking-widest uppercase text-[#FF671F]">
        {label}
      </span>
    ...
    >
      ...
      >
        ...
        <div className="sticky top-0 flex items-center justify-between px-5 py-4 bg-gradient-to-r from-[#1a2744] to-[#293C86] border-b-2 border-[#FF671F] rounded-t-2xl z-10">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center text-white hover:bg-white/20 transition"
          >
            <IconClose />
          </button>
        </div>
        ...
        <div className="p-5 space-y-5">
          ...
          {/* Personal */}
          <div>
            <p className="text-[10px] font-black tracking-widest uppercase text-[#1a2744] mb-3 border-b border-gray-100 pb-1">
              Personal Details
            </p>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-3">
              <Row
                ...
                value={fmt(martyr.martyrdomDate)}
              />
              <Row label="Place" value={martyr.placeOfMartyrdom} />
            </div>
            <Row
              label="Incident Description"
              value={martyr.incidentDescription}
            />
          </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h3 className="text-base font-black text-[#1a2744] text-center">
          Delete Record?
        </h3>
        <p className="text-sm text-gray-500 text-center mt-1">
          Are you sure you want to delete{" "}
          <span className="font-bold text-gray-800">{martyr.fullName}</span>?
          This action cannot be undone.
        </p>
        <div className="flex gap-3 mt-6">
          <button
            onClick={onClose}
            className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-sm font-bold text-gray-500 hover:bg-gray-50 transition"
          >
            Cancel
          </button>
          <button
...
export default function MartyrVeerJawanPage() {
  ...
  // ── Fetch table data ─────────────────────────────────────────────────────────
  const fetchMartyrs = useCallback(
    async (page = 1, searchVal = search) => {
      ...
      try {
        ...
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/martyrs?${params}`,
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess(
        editId
          ? "Martyr profile updated successfully!"
          : "Martyr profile registered successfully!",
      );
      clearForm();
      fetchMartyrs(1);
    } catch (err) {
  ...
  const handleEdit = (martyr) => {
    ...
    setForm({
      ...
      youtubeLink: martyr.youtubeLink || "",
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/martyrs/${deleteMartyr._id}`,
        { method: "DELETE" },
      );
      const data = await res.json();
      if (!res.ok) throw new Error(data.message);
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-4 sm:px-6 py-4 bg-gradient-to-r from-[#1a2744] to-[#293C86] border-b-2 border-[#FF671F]">
            ...
            {editId && (
              <button
                onClick={clearForm}
                className="text-[10px] font-bold px-3 py-1.5 rounded-lg bg-white/10 text-white hover:bg-white/20 transition border border-white/20"
              >
                ✕ Cancel Edit
              </button>
            )}
          ...
          <form onSubmit={handleSubmit} className="p-4 sm:p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              <Field label="Unit / Regiment" required>
                <input
                  className={inputCls}
                  placeholder="e.g. 21 Maratha Light Infantry"
                  value={form.unit}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              ...
              <Field
                label="Incident Description (Cause of Martyrdom)"
                required
                className="col-span-full"
              >
                <textarea
                  className={`${inputCls} resize-y min-h-[100px]`}
                  placeholder="Describe the incident and circumstances of martyrdom..."
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              ...
              <Field label="Number of Sons">
                <input
                  ...
                  className={inputCls}
                  placeholder="0"
                  value={form.numberOfSons}
                  onChange={(e) => set("numberOfSons", e.target.value)}
                />
              </Field>
              <Field label="Parents Details" className="col-span-full">
                <textarea
                  className={`${inputCls} resize-y min-h-[80px]`}
                  placeholder="Father's name, mother's name, contact details, etc."
                  value={form.parentsDetails}
                  onChange={(e) => set("parentsDetails", e.target.value)}
                />
              </Field>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              ...
              <Field label="Taluka / Tehsil" required>
                <input
                  className={inputCls}
                  placeholder="Taluka or tehsil name"
                  value={form.taluka}
              ...
              >
                ...
                {loading
                  ? editId
                    ...
                    : "Registering…"
                  : editId
                    ? "Update Martyr Profile"
                    : "Register Martyr Profile"}
              </button>
            </div>
          </form>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 sm:px-6 py-4 bg-gradient-to-r from-[#1a2744] to-[#293C86] border-b-2 border-[#FF671F]">
            ...
            >
              ...
              <button
                type="submit"
                className="px-3 py-2 rounded-xl bg-[#FF671F] text-white text-xs font-bold hover:bg-[#e55a17] transition min-h-[36px] shrink-0"
              >
                Search
              </button>
              {search && (
                <button
                  ...
                  className="px-2 py-2 rounded-xl bg-white/10 text-white/70 text-xs font-bold hover:bg-white/20 transition min-h-[36px] shrink-0"
                >
                  Clear
                </button>
              )}
              ...
              <table className="w-full text-left border-collapse min-w-[700px]">
                <thead>
                  <tr className="border-b border-gray-100">
                    {[
                      ...
                      <th
                        key={h}
                        className="px-4 py-3 text-[9px] font-black tracking-widest uppercase text-[#1a2744] whitespace-nowrap bg-gray-50/60"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody>
                  {tableData.map((m, i) => (
                    <tr
                      key={m._id}
                      className="border-b border-gray-50 hover:bg-[#293C86]/3 transition group"
                    >
                      {/* # */}
                      ...
                      {/* Full Name */}
                      <td className="px-4 py-3 text-sm font-bold text-[#1a2744] whitespace-nowrap">
                        {m.fullName}
                      </td>
                      {/* Unit */}
                      <td className="px-4 py-3 text-xs text-gray-600 whitespace-nowrap max-w-[160px] truncate">
                        {m.unit}
                      </td>
                      {/* Martyrdom Date */}
                      <td className="px-4 py-3 text-xs text-gray-600 whitespace-nowrap">
                        {fmtDate(m.martyrdomDate)}
                      </td>
                      {/* Place */}
            ...
            <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-t border-gray-100">
              <p className="text-xs text-gray-400">
                ...
                <span className="font-bold text-gray-700">
                  {(tableMeta.page - 1) * tableMeta.limit + 1}–
                  {Math.min(tableMeta.page * tableMeta.limit, tableMeta.total)}
                </span>{" "}
                of{" "}
                <span className="font-bold text-gray-700">
                  {tableMeta.total}
